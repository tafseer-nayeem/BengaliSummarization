...
from Python.baselineAlgorithm.sumy._compat import to_string, to_unicode, unicode
from Python.baselineAlgorithm.sumy.utils import normalize_language
...
class BanglaSentenceTokenizer:
    def tokenize(self,text):
        try:
            from Python.baselineAlgorithm.sentTokenizer import sentTokenizing
        except ImportError as e:
            raise ValueError("Bangla tokenizer requires sentTokenizer. Please, install.")
        sentTokenizing = sentTokenizing()
        return sentTokenizing.sentTokenize(text)
    
class BanglaWordTokenizer:
    def tokenize(self,text):
        try:
            from Python.baselineAlgorithm.wordConverter import word
        except ImportError as e:
            raise ValueError("Bangla tokenizer requires wordConverter. Please, install.")
        word = word()
        return word.sentToWord(text)
...
class Tokenizer(object):
    ...
    SPECIAL_SENTENCE_TOKENIZERS = {
        ...
        'bangla': BanglaSentenceTokenizer()
    }
